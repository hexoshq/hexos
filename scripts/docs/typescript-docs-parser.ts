...
export class TypescriptDocsParser {
    ...
    private parseDeclaration(
        ...
        let category = this.getDocsCategory(statement);
        if (category === undefined) {
            // Auto-derive category from file path for exported declarations
            const derivedCategory = this.deriveCategoryFromPath(sourceFile);
            if (!derivedCategory || !this.isExported(statement)) {
                return;
            }
        ...
        if (ts.isInterfaceDeclaration(statement)) {
            ...
            const parameters = statement.parameters.map(p => {
                ...
                if (name.startsWith('{') && name.endsWith('}')) {
                    // Destructured parameters (e.g. `{ prop1, prop2 }`) cause MDX parse errors
                    // because `{` is interpreted as a JSX expression. Replace with a simple name.
                    name = info.sourceFile.endsWith('.tsx') ? 'props' : 'params';
                }
                return {
                    name,
    ...
    /**
     * Derives a documentation category from the source file path when no @docsCategory tag is present.
     * E.g., "packages/react-core/src/hooks/useAgent.ts" → "react-core/hooks"
     */
    private deriveCategoryFromPath(sourceFile: string): string | undefined {
        const match = sourceFile.match(/packages\/([^/]+)\/src\/(.+)\.[^.]+$/);
        if (!match) return undefined;
        const relativePath = match[2];
        const dir = path.dirname(relativePath);
        if (dir === '.') {
            return undefined; // Top-level src/ files need explicit @docsCategory
        }
        return normalizeForUrlPart(dir);
    }

    /**
     * Checks whether a statement has the export keyword.
     */
    ...
    /**
     * Type guard for the types of statement which can be processed by the doc generator.
     */
