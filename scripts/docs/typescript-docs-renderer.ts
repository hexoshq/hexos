...
import { generateFrontMatter, processReadmeForMdx, titleCase } from './docgen-utils';
...
export class TypescriptDocsRenderer {
    render(pages: DocsPage[], docsUrl: string, outputPath: string, typeMap: TypeMap, readmeContent?: string): number {
        let generatedCount = 0;
        if (!fs.existsSync(outputPath)) {
            fs.ensureDirSync(outputPath);
        }

        // Extract the section base path (e.g., 'common' from '.../pages/docs/common')
        const docsIndex = outputPath.indexOf('/pages/docs/');
        const sectionBasePath = docsIndex !== -1
            ? outputPath.slice(docsIndex + '/pages/docs/'.length)
            : '';
        ...
        // Track pages per category for generating doc page LinkCards in index files
        const categoryPages = new Map<string, Array<{ fileName: string; title: string }>>();
        for (const page of pages) {
            const categoryPath = page.category.join('/');
            if (!categoryPages.has(categoryPath)) {
                categoryPages.set(categoryPath, []);
            }
            if (page.fileName !== 'index') {
                categoryPages.get(categoryPath)!.push({
        ...
        // Track index files that contain declaration content (e.g., AgentRuntime class docs)
        // so the index generator doesn't overwrite them with only LinkCards
        const declarationIndexFiles = new Set<string>();

        for (const page of pages) {
            let markdown = '';
            markdown += generateFrontMatter(page.title);
            markdown += this.renderImports(page);
            ...
            // When a declaration page uses fileName 'index', append LinkCards for sibling pages
            // so the index page contains both the declaration docs AND the navigation
            if (page.fileName === 'index') {
                const categoryPath = page.category.join('/');
                const children = categoryChildren.get(categoryPath);
                const pagesInCategory = categoryPages.get(categoryPath);
                const linkCards: string[] = [];

                if (children && children.size > 0) {
                    for (const child of children) {
                        const basePath = sectionBasePath ? `${sectionBasePath}/` : '';
                        const absolutePath = `/docs/${basePath}${categoryPath}/${child}`;
                        const childName = child;
                        const title = titleCase(childName.replace(/-/g, ' '));
                        linkCards.push(`<LinkCard href="${absolutePath}" title="${title}" />`);
                    }
                }
                if (pagesInCategory && pagesInCategory.length > 0) {
                    for (const docPage of pagesInCategory) {
                        const basePath = sectionBasePath ? `${sectionBasePath}/` : '';
                        const absolutePath = `/docs/${basePath}${categoryPath}/${docPage.fileName}`;
                        const childName = docPage.fileName;
                        const title = titleCase(childName.replace(/-/g, ' '));
                        linkCards.push(`<LinkCard href="${absolutePath}" title="${title}" />`);
                    }
                }
                ...
                if (linkCards.length > 0) {
                    linkCards.sort((a, b) => a.localeCompare(b));
                    markdown += '\n---\n\n';
                    markdown += linkCards.join('\n') + '\n';
                }

                const indexFilePath = path.join(categoryDir, 'index.mdx');
                declarationIndexFiles.add(indexFilePath);
            }
            ...
            for (const subCategory of page.category) {
                ...
                // Skip if this index file was already written with declaration content
                if (declarationIndexFiles.has(indexFile)) {
                    continue;
                }
                ...
                // Also add LinkCards for individual doc pages in this category
                const pagesInCategory = categoryPages.get(categoryPath);
                if (pagesInCategory && pagesInCategory.length > 0) {
                    for (const docPage of pagesInCategory) {
                        const basePath = sectionBasePath ? `${sectionBasePath}/` : '';
        ...
        if (sectionBasePath && sectionRootChildren.size > 0) {
            ...
            if (!hasCustomContent) {
                ...
                for (const child of sectionRootChildren) {
                    const absolutePath = `/docs/${sectionBasePath}/${child}`;
                    newLinkCards.add(absolutePath);
                }
    ...
    private renderMembers(
        ...
        for (const member of members || []) {
            ...
            output += `\n### ${member.name}\n\n`;
            const attrs = [`kind="${member.kind}"`, `type={\`${type}\`}`];
            if (defaultParam) attrs.push(defaultParam.trim());
            if (sinceParam) attrs.push(sinceParam.trim());
            if (experimentalParam) attrs.push(experimentalParam);
            output += `<MemberInfo ${attrs.join(' ')} />\n\n`;
    ...
    private renderImports(page: DocsPage): string {
        const imports: string[] = [];
        imports.push(`import { GenerationInfo } from '@/docs/components/docs/GenerationInfo'`);
        const hasMemberInfo = page.declarations.some(d => {
            if ('members' in d && d.members && d.members.length > 0) return true;
            if ('parameters' in d && d.parameters && d.parameters.length > 0) return true;
            return false;
        });
        if (hasMemberInfo) {
            imports.push(`import { MemberInfo } from '@/docs/components/docs/MemberInfo'`);
        }
        if (page.fileName === 'index') {
            imports.push(`import { LinkCard } from '@/docs/components/docs/LinkCard'`);
        }
        return '\n' + imports.join('\n') + '\n\n';
    }
    ...
    private renderDescription(description: string, knownTypeMap: TypeMap, docsUrl: string): string {
        ...
        // Escape remaining {variable} patterns that MDX would interpret as JSX expressions,
        // but only outside of fenced code blocks (``` ... ```)
